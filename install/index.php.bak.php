...
$verMsg = ' V5.7 GBKSP1';
$s_lang = 'gb2312';
$dfDbname = 'dede_novel1_'.date("Ymd",time());
$errmsg = '';
...
else if($step==3)
{
    ...
    if(!empty($_SERVER['HTTP_HOST']))
	{
        ...
        $cfg_wapurl = 'http://'.str_replace("www.","m.",$_SERVER['HTTP_HOST']);
		$adminmail='admin@'.str_replace("www.","",$_SERVER['HTTP_HOST']);
	}
    else
	{
        $baseurl = "http://".$_SERVER['SERVER_NAME'];
        $cfg_wapurl = "http://".str_replace("www.","m.",$_SERVER['SERVER_NAME']);
		$adminmail='admin@'.str_replace("www.","",$_SERVER['SERVER_NAME']);
	}
...
else if($step==4)
{
    ...
    mysqli_query($conn, $cquery);
    $cquery = "Update `{$dbprefix}sysconfig` set value='{$cfg_wapurl}' where varname='cfg_wapurl';";
    mysqli_query($conn, $cquery);
    ...
    if(!isset($modules) || !is_array($modules))
    {
		...
		@rename("module-install.php","module-install.php.bak");
...
else if($step==10)
{
    ...
    if($conn)
    {
		if(empty($dbname)){
			...
			$info = mysqli_select_db($conn, $dbname)?"<font color='green'>信息正确，可以正常连接该数据库</font>":"<font color='red'>请确保您有创建数据库的权限，否则将无法安装成功！</font>";
			echo $info;
		}
    }
    else
