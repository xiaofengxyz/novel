        ...
        foreach($ks as $k)
        {
            ...
            if($this->ChannelType < 0 || $this->ChannelTypeid < 0){
                ...
                if($this->SearchType=="title"){
					...
					if($stid!="")
					{
						$keyword = cn_substr($this->Keyword, 50);
						$row = $this->dsql->GetOne("SELECT spwords FROM `#@__search_keywords` WHERE keyword='".addslashes($keyword)."'; ");
						if(!is_array($row))
						{
							$inquery = "INSERT INTO `#@__search_keywords`(`keyword`,`spwords`,`count`,`result`,`lasttime`) VALUES ('".addslashes($this->Keyword)."', '".addslashes($this->Keywords)."', '1', '0', '".time()."'); ";
